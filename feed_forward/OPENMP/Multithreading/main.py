...
logger.add("Mult_profiling.log", format="{time} {level} {message}", level="INFO", rotation="1 week")

def task():
    # logger.info("Starting CUDA kernel execution.")
    # Run the kernel
    for _ in range(1000):
        lib.feed_forward_mult(N, input_data, weights, output, num_threads)    # logger.info("CUDA kernel execution completed.")
...
if __name__ == "__main__":
    ...
    lib.feed_forward_mult.argtypes = [ctypes.c_int, 
                                np.ctypeslib.ndpointer(dtype=np.float32, ndim=1, flags='C_CONTIGUOUS'),
                                np.ctypeslib.ndpointer(dtype=np.float32, ndim=1, flags='C_CONTIGUOUS'),
                                np.ctypeslib.ndpointer(dtype=np.float32, ndim=1, flags='C_CONTIGUOUS'),
                                ctypes.c_int]
    lib.feed_forward_mult.restype = None
