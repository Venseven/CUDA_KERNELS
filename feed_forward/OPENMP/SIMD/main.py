...
logger.add("SIMD_profiling.log", format="{time} {level} {message}", level="INFO", rotation="1 week")

def task():

    for _ in range(1000):
        lib.feed_forward_simd(N, input_data, weights, output)
...
if __name__ == "__main__":
    ...
    lib.feed_forward_simd.argtypes = [ctypes.c_int, 
                                np.ctypeslib.ndpointer(dtype=np.float32, ndim=1, flags='C_CONTIGUOUS'),
                                np.ctypeslib.ndpointer(dtype=np.float32, ndim=1, flags='C_CONTIGUOUS'),
                                np.ctypeslib.ndpointer(dtype=np.float32, ndim=1, flags='C_CONTIGUOUS')]
    lib.feed_forward_simd.restype = None
